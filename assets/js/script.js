(function ($) {
	...
	$ttContactForm.submit(function (e) {
		...
		$cfmContainerInner.html('<span class="tt-cfm-sending">Sending your message...</span>');

		const formData = $(this).serialize();

		$.ajax({
			type: "POST",
			url: "https://formsubmit.co/ajax/admin@hyperpath.co.uk",
			data: formData,
			dataType: "json",
		})
			...
			.fail(function () {
				$cfmContainerInner.html('<span class="tt-cfm-error">Error: Unable to send message. Try again later.</span>');
				scrollToContactFormTop();
			});
			...
			if (!tt_isMobile) {
				if ($("body").hasClass("tt-smooth-scroll")) {
					...
					lenis.scrollTo(cfmTopY, {
						duration: 1,
						easing: (x) =>
							Math.min(x < 0.5 ? 8 * x * x * x * x : 1 - Math.pow(-2 * x + 2, 4) / 2),
					});
				} else {
